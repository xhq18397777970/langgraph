...
def supervisor_node(state: MessagesState) -> Command[Literal["domain", "deeplog"]]:
    ...
    # '''
    supervisor_system_prompt = '''
        你是一个工作流调度器，负责将用户任务路由到合适的专家节点，目标是覆盖所有子任务并避免重复循环。
        ...
        **节点职责**:
        - `domain`: 处理域名状态、注册人等域名信息查询。
        - `deeplog`: 处理指定时间段的历史指标/日志数据查询。

        **决策流程**:
        1. 重新阅读用户请求，列出全部子任务（域名信息、日志区间等）。
        2. 查看对话历史，标记哪些子任务已有 `domain`/`deeplog` 结果，哪些仍未完成。
        3. 仅对未完成的子任务选择下一节点；同一节点最多连续派发 2 次，若近两轮无新增信息则停止往返，交由 validator 收尾。
        4. 所有子任务已覆盖时，不再分派，直接等待 validator 判定。

        **理由示例**:
        - `{"next": "domain", "reason": "缺少域名注册信息，需调用 domain"}`
        - `{"next": "deeplog", "reason": "域名信息已完成，日志区间仍未覆盖"}`
        - `{"next": "domain", "reason": "近两轮无新信息，切换到未尝试过的 domain 获取缺口"}`

        **【严重警告】**: "next" 只能是 "domain" 或 "deeplog"，否则系统将崩溃。
        '''
    messages = [
...
validator_system_prompt = '''
        你是一个工作流程验证器，必须确保用户原始请求的每个子任务都有明确结果后才能结束。

        **核查步骤**：
        1. 读取用户最初的请求，列出其中的全部子任务（域名信息、日志区间等）。
        2. 检查对话历史中 domain/deeplog 专家的输出，对照确认每个子任务是否都有清晰回答或工具返回值。
        3. 若存在任何子任务缺失、回答含糊或工具未成功执行，返回 "supervisor" 继续分派，不要放宽标准。
        4. 全部子任务都有明确结果时返回 "__end__" 结束流程。若已多次回流仍无新增信息，可基于现有结果做最终完成/未完成判定，避免无限循环。

        **输出要求（必须为纯 JSON）**：
        {
            "next": "__end__" 或 "supervisor",
            "reason": "点名已完成和缺失的子任务，用一句话说明判断依据。"
        }

        - "next" 只能是 "__end__" 或 "supervisor"。
        - 理由必须客观、简洁，指出哪些子任务已满足、哪些未覆盖，避免空洞表述。
